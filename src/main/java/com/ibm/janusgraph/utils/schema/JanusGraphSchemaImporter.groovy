...
class IndexBean {
    ...
    public void make(JanusGraphManagement mgmt, boolean isVertexIndex) {
        ...
        for (property in propertyKeys) {
            if (mixedIndex != null) {
//                ib.addKey(mgmt.getPropertyKey(property),Mapping.TEXTSTRING.asParameter(),Parameter.of(ParameterType.TEXT_ANALYZER.getName(),"whitespace"))

                if ("zfz_registered_capital_rmb".equals(property)){
                    ib.addKey(mgmt.getPropertyKey(property))
                    println "[ add specical ] : ${property} "
                }else {
                    ib.addKey(mgmt.getPropertyKey(property),Mapping.TEXTSTRING.asParameter())
                }
            }else {
                ib.addKey(mgmt.getPropertyKey(property))
            }
